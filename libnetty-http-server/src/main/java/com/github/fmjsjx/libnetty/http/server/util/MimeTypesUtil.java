package com.github.fmjsjx.libnetty.http.server.util;
...
public class MimeTypesUtil {
    ...
    /**
     * Probe the content type of the specified file path.
     *
     * @param filePath the path of the file to probe
     * @return the file's MIME type
     */
    public static CharSequence probeContentType(Path filePath) {
        return MimeTypesUtil.getMimeType(filePath.getFileName().toString());
    }
    ...
    private record MimeType(AsciiString typeString, List<String> extensions) {
        ...
        private static MimeType parse(String line) {
            ...
            return new MimeType(AsciiString.cached(split[0].intern()), List.of(Arrays.copyOfRange(split, 1, split.length)));
        }
